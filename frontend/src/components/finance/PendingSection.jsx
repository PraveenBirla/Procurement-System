...
export const PendingSection = () => {
  ...
  // Helper to commit backend action when timer expires
  const commitBackendAction = async (id, decision, remarks) => {
    ...
    try {
      await requisitionService.financeUpdate(id, payload);

      setRequisitions((prev) => prev.filter((r) => r.id !== id));
      removeStoredTimer(id);
    } catch (err) {
      console.error("Failed to commit delayed finance decision:", err);
  ...
  return (
    <div className="admin-requisition-section">
      <div className="section-header">
        <div>
          <h2 className="section-title">Finance Pending Requisitions</h2>
          <p className="section-subtitle">
            Requisitions waiting for finance review and decision.
          </p>
        </div>
        <button
        ...
        <table className="table">
          ...
          <tbody>
            {loading ? (
              ...
              <tr>
                <td colSpan="8" className="no-data">
                  No Pending Finance Requisitions
                </td>
              </tr>
            )}
          </tbody>
              ...
              <form onSubmit={handleSubmitAction} noValidate>
                ...
                <div className="modal-actions">
                  <button
                    ...
                    disabled={submittingAction}
                  >
                    {submittingAction ? "Submitting...": actionModal.type ===
                      "approved"
                      ? "Confirm Approve"
                      : "Confirm Reject"}
                  </button>
