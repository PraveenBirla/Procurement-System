...
const FINANCE_DECIDED_STATUSES = ["APPROVED", "REJECTED"];
...
export const  RequisitionSection = () => {
  ...
  const loadRequisitions = async () => {
    ...
    try {
      ...
      const decidedResults = await Promise.all(
        FINANCE_DECIDED_STATUSES.map((status) =>
          requisitionService.getFinanceRequisitionsByStatus(status)
        )
      );
  ...
  const handleSubmitAction = async (e) => {
    ...
    try {
      await requisitionService.financeUpdate(req.id, payload);
  ...
  return (
    <div className="admin-requisition-section">
      <div className="section-header">
        <h2 className="section-title">Finance Requisitions</h2>
      </div>
