...
export const InDeliverySection = () => {
  ...
  const loadOrders = async () => {
    setLoading(true);
    try {
      const res = await suppliersService.getSuppliersOrderByStatus(IN_DELIVERY_STATUS);
      setOrders(res);
      setError("");
    } catch (err) {
  ...
  const handleSendInvoice = async (po) => {
    setSendingInvoiceId(po.id);
    try {
      await purchaseOrderService.updateStatus(po.id, { status: DELIVERED_STATUS });
      await loadOrders();
    } catch (err) {
      setError(getErrorMessage(err));
    } finally {
  ...
  return (
    <div className="admin-requisition-section">
      <div className="section-header">
        <h2 className="section-title">In Delivery</h2>
      </div>
        ...
        <table className="table">
          ...
          <tbody>
            {loading ? (
              ...
              orders.map((po) => {
                const canSendInvoice = po.status === IN_DELIVERY_STATUS;
                return (
                  <tr key={po.id}>
                      ...
                      <div className="action-group">
                        ...
                        {canSendInvoice && (
                          <button
                            ...
                            onClick={() => handleSendInvoice(po)}
                            disabled={sendingInvoiceId === po.id}
                          >
                            {sendingInvoiceId === po.id ? "…" : "Send Invoice"}
                          </button>
                        )}
