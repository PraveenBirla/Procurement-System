...
const SUPPLIER_ACCEPTED_STATUS = "PO_RECEIVED";

export const AcceptedOrderSection = () => {
  ...
  const loadOrders = async () => {
    setLoading(true);
    try {
      const res = await suppliersService.getSuppliersOrderByStatus(SUPPLIER_ACCEPTED_STATUS);
      setOrders(res);
      setError("");
    } catch (err) {
  ...
  const handleGenerateInvoice = async (po) => {
    ...
    try {
      await purchaseOrderService.generateInvoiceOrder(po.id);
      await loadOrders();
    } catch (err) {
      setError(getErrorMessage(err));
    } finally {
        ...
        <table className="table">
          ...
          <tbody>
            {loading ? (
              ...
              orders.map((po) => {
                const canGenerateInvoice = po.status === SUPPLIER_ACCEPTED_STATUS;
                return (
                  <tr key={po.id}>
                      ...
                      <div className="action-group">
                        ...
                        {canGenerateInvoice && (
                          <button
                            ...
                            disabled={generatingInvoiceId === po.id}
                          >
                            {generatingInvoiceId === po.id ? "…" : "Generate Invoice"}
                          </button>
                        )}
